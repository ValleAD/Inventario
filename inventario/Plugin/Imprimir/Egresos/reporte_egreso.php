...
$idusuario = $_SESSION['iduser'];
 if (isset($_POST['vale'])) {?>
        <h3 style="text-align: center;font-size: 14px;">MINISTERIO DE SALUD</h3>
<h3 style="text-align: center;font-size: 14px;">HOSPITAL NACIONAL SANTA TERESA</h3>
<h4 style="text-align: center;font-size: 14px;">UNIDAD DE ADQUISICIONES Y CONTRATACIONES</h4>
<h4 style="text-align: center;font-size: 14px;">INSTITUCIONAL</h4>
<h4 style="text-align: center;font-size: 14px;">EGRESOS DE VALE</h4>
          <div id="h">
            <table class="table">
...
<?php
if ($tipo_usuario==1) {
    $sql = "SELECT * FROM `detalle_vale` D JOIN `tb_vale` V ON D.numero_vale=V.CodVale GROUP by codigo,descripcion,cantidad_despachada,precio,descripcion,unidad_medida";
 }
 if ($tipo_usuario==2) {
$sql = "SELECT * FROM `detalle_vale` D JOIN `tb_vale` V ON D.numero_vale=V.CodVale WHERE V.idusuario='$idusuario' GROUP by codigo,descripcion,cantidad_despachada,precio,descripcion,unidad_medida";
 }
