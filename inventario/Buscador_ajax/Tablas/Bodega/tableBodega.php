   ...
   #div{
    display: block;
        ...
        <tr>
            ...
            <?php }else{?>
                <button type="button" class="btn btn-danger" style="opacity: .7;cursor: no-drop;">Ver Detalles</button>
            <?php } ?>
            </form> 
        </div>
            </td>
...
<div id="div" style = " max-height: 442px;  overflow-y:scroll;overflow-x:none;">
    <table class="table">
    <tbody><?php 
   $sql = "SELECT * FROM tb_bodega  where idusuario='$idusuario' and   Order by $columna $tipo";
    $result = mysqli_query($conn, $sql);
$n=0;
    while ($solicitudes = mysqli_fetch_array($result)){ 
         $des=$solicitudes['departamento'];
